...
const ParkingDemo = () => {
    ...
    const handleFetch = async () => {
        const updatedParkingLots = {...parkingLots};
        try {
            const response = await axios.post('http://localhost:8080/api/parkinglotManager/fetch', {"plateNumber": plateNumber});
            const fetchedData = response.data;
            for (const lot in fetchedData) {
                updatedParkingLots[lot] = fetchedData[lot].map((car) => car && car.plateNumber === plateNumber ? null : car ? car.plateNumber : null);
            }
            setParkingLots(updatedParkingLots);
        } catch (error) {
            console.error('Error fetching parking data:', error);
